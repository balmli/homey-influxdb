...
module.exports = class InfluxDb {
    ...
    async _pingDatabaseV1(host, protocol, port) {
        ...
        if (!result || !result.response || result.response.statusCode !== 204) {
        ...
        try {
            ...
            if (!this._hostDefined()) {
                ...
                try {
                    ...
                    this.log(`InfluxDb: connected to ${this._host}:${this._port} ${this._database}`);
                } catch (err) {
                    this.log(`InfluxDb: connection to ${this._host}:${this._port} ${this._database} failed!`);
                    this._connected = false;
                }
            }
        } catch (err) {
        ...
        return measurements
            .map(({measurement, tags = {}, fields, timestamp}) => {
                ...
                const tagsString = Array.isArray(tags) ?
                    tags.map(tag => `,tag_${cntr++}=${escape.tag(tag)}`).join('') :
                    Object.keys(tags).map(tagKey => `,${tagKey}=${escape.tag(tags[tagKey])}`).join('');
