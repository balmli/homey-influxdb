...
module.exports = class InfluxDb {
    ...
    async _pingDatabase(host, port, username, password) {
    ...
    async _createInfluxDb() {
        try {
            ...
            if (!this._hostDefined()) {
                ...
                try {
                    await this._pingDatabase(this._host, this._port, this._username, this._password);
    ...
    async _checkInfluxDbConnection() {
        ...
        let online = false;
        try {
            await this._pingDatabase(this._host, this._port, this._database, this._password);
            online = true;
        } catch (err) {
            online = false;
        }
