...
module.exports = class DeviceHandler extends EventEmitter {
    ...
    async _onDeviceCreate(deviceAdded) {
        ...
        this._addToFetchQueue(deviceAdded);
    }
    ...
    async _onDeviceUpdate(deviceUpdated) {
        ...
        if (changedDevice) {
            ...
            this._addToFetchQueue(deviceUpdated);
        }
    }
    ...
    async _addToFetchQueue(theDevice) {
        this._fetchQueue.set(theDevice.id, theDevice.id);
        if (this._getRegisterTimeout) {
            clearTimeout(this._getRegisterTimeout);
        }
        this._getRegisterTimeout = setTimeout(this._onFetchAndRegister.bind(this), 20000);
    }

    async _onFetchAndRegister() {
        const fetchQueue = Array.from(this._fetchQueue.keys());
        this._fetchQueue.clear();
        this.log(`_onFetchAndRegister: ${fetchQueue.length} to be registered`);
        for (let id of fetchQueue) {
            const device = await this._api.devices.getDevice({ id: id });
            if (device) {
                await this._registerDevice(device);
            } else {
                this.log(`_onFetchAndRegister: no such device: ${id}`);
            }
        }
    }
